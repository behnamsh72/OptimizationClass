...
def get_edges_from_map_data(map_filename):
    ...
    edges = list()
    for line in map_data:
        edges.append(get_edge_from_line(line))

    return edges


def get_edge_from_line(line):
    l = line.strip('\n').split(' ')
    ...
    if start not in digraph.get_nodes() and end not in digraph.get_nodes():
        ...
        for each in digraph.get_edges_for_node(start):
            if each.get_destination() not in path:
                new_path = get_best_path(digraph, each.get_destination(), end, path, max_dist_outdoors, best_dist,
                                         best_path)
                if new_path != None:
