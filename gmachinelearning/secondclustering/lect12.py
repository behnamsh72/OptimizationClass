...
# this is a method called z-scaling if for example one feature is weight and i clustered them
# well weight would always completely dominate the Euclidean distance Because the dynamic range of the weights
# in this room is much higher than the dynamic range of 0 to 1.and so for the reptiles in in previous lectures
# we change number of legs to  a binary variable but maybe we don't wanna make weight a binary variable.
# because it's something we want to take into account.so what we do is we scale it.so this is a method called
# Z scaling More general than just making 0 or 1.it takes in all of the values of specific feature and then perform
# some simple calculations and then when it's done,the resulting array  it returns has a known mean and a known
# standard deviation.So what's the mean going to  be?it's always going to be the same thing,independent of the initial
# values. the mean always is 0 and the standard deviation a little harder to figure,but it will always be 1.
# The other way people scale is interpolate.They take the smallest value and call it 0, the biggest value they call it 1.
# and then they do  a linear interpolation of all the values betwen 0 and 1.So the range is 0 to 1.
def scaleAttrs(vals):
